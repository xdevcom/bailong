...
const bailongVersion = [2, 3000, 1035194821];
...
/**
 * utility that fetches latest bailong version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
export const fetchLatestBailongVersion = async (options = {}) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Boom(`Failed to fetch latest bailong version: ${response.statusText}`, { statusCode: response.status });
        }
...
/**
 * Stream errors generally provide a reason, map that to a bailong DisconnectReason
...
export const fetchLatestBaileysVersion = fetchLatestBailongVersion;
