...
export const makeChatsSocket = (config) => {
    ...
    const resyncAppState = ev.createBufferedFunction(async (collections, isInitialSync) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            let _syncLoopIterations = 0;
            const _MAX_SYNC_LOOP_ITERATIONS = 100;
            while (collectionsToHandle.size && ++_syncLoopIterations <= _MAX_SYNC_LOOP_ITERATIONS) {
                const states = {};
                const nodes = [];
    ...
    /**
     * Enable/Disable link preview privacy, not related to bailong link preview generation
     */
